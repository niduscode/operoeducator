...
import { Curso, Horario, SUCURSALES, Sucursal } from "@/lib/types";
import { AlumnoInput } from "@/lib/firestore";

interface BulkImportProps {
  onCancel: () => void;
  onImport: (valid: AlumnoInput[]) => Promise<void>;
}

interface ParsedRow {
  raw: Record<string, string>;
  data: AlumnoInput | null;
  errors: string[];
  rowIndex: number;
}
...
// Mapa de columnas flexibles → campo interno.
// Todo normalizado a lowercase sin acentos para comparar.
const HEADER_ALIASES: Record<string, "nombre" | "telefono" | "curso" | "turno" | "sucursal"> = {
  nombre: "nombre",
  name: "nombre",
...
function normalizeCurso(v: string): Curso | null {
  const n = stripAccents(v.trim().toLowerCase());
  const match = CURSOS.find((c) => stripAccents(c.toLowerCase()) === n);
  return match ?? null;
}

function normalizeHorario(v: string): Horario | null {
  const n = stripAccents(v.trim().toLowerCase());
  const match = HORARIOS.find((h) => stripAccents(h.toLowerCase()) === n);
  return match ?? null;
}
...
function parseRow(raw: Record<string, string>, rowIndex: number): ParsedRow {
  ...
  const curso = cursoRaw ? normalizeCurso(cursoRaw) : null;
  if (!curso) errors.push(`curso inválido ("${cursoRaw}")`);

  const turnoRaw = (raw.turno ?? "").trim();
  const horario = turnoRaw ? normalizeHorario(turnoRaw) : null;
  if (!horario) errors.push(`turno inválido ("${turnoRaw}")`);
  ...
  const data: AlumnoInput | null =
    errors.length === 0 && curso && horario && sucursal
      ? {
          nombre,
          telefono,
          sucursal,
...
// Traduce un array de objetos "crudos" (columnas tal como vienen del archivo)
// a un array de ParsedRow con los campos canonizados.
function parseRawSheet(rows: Record<string, string>[]): ParsedRow[] {
  return rows.map((row, i) => {
    ...
    return parseRow(canonical, i + 2); // +2 = header está en fila 1, datos desde 2
  });
}

export default function BulkImport({ onCancel, onImport }: BulkImportProps) {
        ...
        <div className="space-y-3">
          <div className="p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-sm text-slate-600">
            ...
            <p className="font-mono text-xs bg-white p-2 rounded-lg border border-slate-200">
              nombre · telefono · curso · turno · sucursal
            </p>
            <p className="text-xs text-slate-500 mt-2">
              Los nombres de columna aceptan variantes (mayúsculas, con o sin
              tilde). Los alumnos importados quedan sin profe guía asignado.
            </p>
          </div>
              ...
              <table className="w-full text-xs min-w-[600px]">
                <thead>
                  <tr className="text-left text-[10px] text-slate-400 uppercase tracking-widest border-b border-slate-100">
                    ...
                    <th className="pb-2 pr-2">Turno</th>
                ...
                <tbody>
                  {rows.slice(0, 20).map((r) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-2">{r.raw.curso || ""}</td>
                      <td className="py-2 pr-2">{r.raw.turno || ""}</td>
